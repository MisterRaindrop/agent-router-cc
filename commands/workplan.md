---
description: Turn an approved Design into the work plan -- steps, task breakdown, dependencies, verification, rollout; the user reviews a summary and approves
allowed-tools: Bash, Read, Write, Edit, Task, AskUserQuestion
---
Translate an **approved `DESIGN.md`** into `WORKPLAN.md` -- the *how*: implementation steps,
...
`WORKPLAN.md` lives next to the Design (`PLAN.md` is still read for plans written before the rename). Its frontmatter is the stage record `/router:go` trusts
