...
- It spends **~5.5x more tokens**, but on a model that should be materially cheaper
  per token.

Whether router is cheaper **per verified task** therefore hinges on the codex-vs-Opus
price ratio: router wins iff codex's effective $/token is more than ~5.5x below
